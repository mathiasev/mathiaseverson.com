        ...
        <div className="bg-white py-24 sm:py-32">
            <div className="mx-auto grid max-w-7xl grid-cols-1 gap-x-8 gap-y-12 px-6 sm:gap-y-16 lg:grid-cols-2 lg:px-8">
                <article className="mx-auto w-full max-w-2xl lg:mx-0 lg:max-w-lg">
                    <time dateTime={featuredPost.date.toISOString()} className="block text-sm leading-6 text-slate-600">
                        {featuredPost.date.toLocaleDateString('en-AU')}
                    </time>
                    <h2 id="featured-post" className="mt-4 text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
                        {featuredPost.title}
                    </h2>
                    <p className="mt-4 text-lg leading-8 text-slate-600">{featuredPost.summary}</p>
